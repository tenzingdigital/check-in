...
router.post('/:id/safeguarding-alert', wrap(async (req, res) => {
  ...
  const row = await db.withIdentity(req.session.userId, async (client) => {
    ...
    if (on && req.session.role === 'admin') {
      const { rows: [target] } = await client.query('select role from profiles where id = $1', [id]);
      if (target && (target.role === 'guard' || target.role === 'kiosk')) {
        throw new HttpError(400, 'A guard or a self check-in tablet cannot receive the nightly email. Promote them to supervisor or admin first.');
      }
    }
    const { rows } = await client.query(
    ...
    // Ticking someone back on is also the admin's way of undoing an
    // unsubscribe (049): the row is what the staff card shows, so it must go.
    // Every kind that reads this tick (054: the nightly email, and the two
    // retired kinds — the overnight safeguarding alert and the House Rules
    // reminder — whose rows are history under "(now the nightly email)"),
    // or the tick and a row would disagree the moment the admin looked
    // away.
    if (on && rows[0]) {
