...
(041) are one nightly email of counts, one of which — children on site
without a guardian — links to a report that names the household behind
the login. The owner ruled against a separate evening email that would
...
| Child-supervision arrangements | `supervision_arrangements` (migration 053, only where the centre turns households on) | The Appendix 5 fact: which household's children, which adult resident is minding them (`carer_id`), from and to, whether overnight was approved, who recorded it and when. No contact number, no free text — the signed paper form keeps the rest | All staff, at the door: it is where the fact is acted on. Kept as long as the register, then purged; erasing the carer (or the household itself) removes it; erasing a household member leaves it, since it names no child, only the household; on the audit trail |
...
The other residual risk is the search itself: it needs only two letters, so
...
| Changes to residents, staff and settings with the row before and after; every register correction (the removed entry in full, who removed it, why; every entry added by hand) | `admin_audit` | Accountability for changes to the records, and the evidence behind a corrected register | Same period as the register; purged nightly |
...
reinstate by ticking the person again. Login codes, invitations and
