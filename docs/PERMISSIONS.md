...
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| Who am I (session, settings, feature switches)<br><small>A kiosk session may read this too, so the tablet can learn its own role and the site name</small><br><code>GET /api/session</code> | — | ✓ | ✓ | ✓ | ✓ | ✓ |
...
| Ask for a password-reset link<br><small>Same answer whether or not the address exists; mounted ahead of the kiosk gate like every other unauthenticated route</small><br><code>POST /api/password-reset</code> | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
...
| Correct a staff member's name<br><code>POST /api/staff/:id/name</code> | — | ✗ | ✗ | ✗ | ✓ | ✓ |
| Tick or untick whether a staff member receives the weekly report<br><code>POST /api/staff/:id/weekly-report</code> | — | ✗ | ✗ | ✗ | ✓ | ✓ |
| Tick or untick whether a staff member receives the nightly email<br><code>POST /api/staff/:id/safeguarding-alert</code> | — | ✗ | ✗ | ✗ | ✓ | ✓ |
