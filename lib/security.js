...
const PAGES = ['index.html', 'checkin.html', 'admin.html', 'org.html', 'help.html', 'kiosk.html'];

function inlineHashes(pattern) {
...
function inlineScriptHashes() {
  return inlineHashes(/<script(?![^>]*\bsrc=)[^>]*>([\s\S]*?)<\/script>/g);
}

// Each page's own <style> block is admitted the same way. Without this the
// block is refused outright once 'unsafe-inline' is gone — silently, as a
// page that looks unstyled rather than an error — which is exactly what
// happened to the admin page's Buildings tab on 4 September 2026.
function inlineStyleHashes() {
  return inlineHashes(/<style[^>]*>([\s\S]*?)<\/style>/g);
}
...
module.exports = { securityHeaders, buildCsp, cspAllowingForms };
