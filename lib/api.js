...
// contents. These are messages the RPC authors wrote for a guard to read.
const USER_FACING_SQLSTATES = new Set([
  ...
  '23505', // unique_violation — "A ... within a minute of that time is already on the register"
]);

// A unique_violation an RPC raised on purpose carries a message written for
// a person; one Postgres raised itself carries the constraint's name, and
// that must not reach the browser. Postgres sets err.constraint only on the
// latter.
function duplicateMessage(err) {
  return err.constraint ? 'Already on the record' : err.message;
}

function translateDbError(err) {
  // A refusal is a 403, whichever route it surfaces through: the permission
  // matrix (test/permissions.js) holds every route to that. Not found and a
  // duplicate get their own HTTP statuses; the rest are the caller's
  // mistake, 400.
  if (err.code === '42501') return new HttpError(403, err.message);
  if (err.code === 'P0002') return new HttpError(404, err.message);
  if (err.code === '23505') return new HttpError(409, duplicateMessage(err));
