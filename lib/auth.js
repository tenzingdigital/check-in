...
async function signIn(email, password, { ip, userAgent, deviceToken } = {}) {
  ...
  return withOwner(async (client) => {
    ...
    // The second step (migration 021): supervisors and admins at a site with
    // the switch on type a code that was emailed to them — unless this
    // device was trusted within the last thirty days.
    const { rows: need } = await client.query(`select auth.mfa_required_for($1) as required`, [rows[0].id]);
    if (need[0]?.required && !(await deviceTrusted(client, rows[0].id, deviceToken))) {
      ...
      const { rows: ch } = await client.query(
        ...
        [rows[0].id, tokenHash(code), MFA_CODE_MINUTES, String(userAgent || "").slice(0, 300)],
      );
      const { rows: who } = await client.query(`select email from auth.users where id = $1`, [rows[0].id]);
      const sent = await mail.send({ to: who[0].email, ...mail.codeEmail({ fullName: profile[0].full_name, code, minutes: MFA_CODE_MINUTES }) });
      await noteLogin(client, { email: clean, userId: rows[0].id, outcome: "mfa_sent", ip, userAgent });
      return { mfaRequired: true, challengeId: ch[0].id, emailHint: maskEmail(who[0].email), delivered: sent.delivered };
    }

    await noteLogin(client, { email: clean, userId: rows[0].id, outcome: "ok", ip, userAgent });
    return createSession(client, rows[0].id, userAgent);
  });
}
...
async function completeMfa({ challengeId, code, trustDevice, ip, userAgent } = {}) {
  ...
  return withOwner(async (client) => {
    ...
    if (trustDevice) {
      ...
      await client.query(
        `insert into auth.mfa_devices (token_sha256, user_id, expires_at, user_agent)
         values ($1, $2, now() + make_interval(days => $3), $4)`,
        [tokenHash(deviceToken), ch.user_id, DEVICE_TTL_DAYS, String(userAgent || "").slice(0, 300)],
      );
    }
