...
// The kiosk tablet is the one exception to that rule, not a loosening of it.
// It is unattended and left running for days at a time (Guided Access, no
// staff member re-opening a browser), so a 12-hour session would show the
// staff login screen on the resident-facing tablet twice a day. It is also
// the session it is safest to leave long-lived: requireSession (below)
// restricts the `kiosk` role to exactly two routes (routes/kiosk.js) plus
// reading/ending its own session, so a stolen cookie for this role can only
// search residents and record check-ins — never see the register, settings,
// or anyone's history — which is what makes thirty days defensible where it
// would not be for a guard.
const KIOSK_SESSION_TTL_DAYS = Number(process.env.HUT_KIOSK_SESSION_DAYS || 30);
...
function throttleKey(email, ip) {
  // trim() is load-bearing, not tidiness. signIn() trims before it looks the
  // account up, so it recorded failures under the trimmed key while
  // routes/session.js checked lockedOut() with the raw body value. A single
  // leading space therefore verified against the real account and counted
  // against a key nobody ever wrote — the lockout never fired, from one IP,
  // for as many guesses as you liked. Normalising in one place is what makes
  // "checked" and "recorded" the same key. The MFA and password-reset keys
  // pass literals through here too, which trim leaves alone.
...
async function signIn(email, password, { ip, userAgent, deviceToken, country } = {}) {
  ...
  return withOwner(async (client) => {
    ...
    // NOT cleared here. Clearing on a correct password meant an attacker who
    // already has the password — the exact threat the second factor is for —
    // could loop this endpoint forever: each pass reset the lockout, minted a
    // fresh challenge, emailed the victim another code, and bought five more
    // guesses. The throttle is now cleared only when a real session is issued
    // (below, and in completeMfa), so password-correct-but-MFA-pending still
    ...
    // HUT_HOME_COUNTRY_CHECK=off records the country on the login and refuses
    // nobody for it. The switch exists because the bundled GeoLite table put
    // an Irish phone outside Ireland on 8 September 2026 and locked the
    // administrator out of her own site; unknown was never treated as abroad,
    // but wrong is worse than unknown. Leave it off until the lookup is
    // trusted again (see lib/geo.js), then delete the variable.
    const checkCountry = process.env.HUT_HOME_COUNTRY_CHECK !== "off";
    const abroad = checkCountry && !!country && !home.includes(country);
    ...
    if (needCode && !(await deviceTrusted(client, userId, deviceToken))) {
      ...
      const { rows: ch } = await client.query(
        `with retired as (
           -- One live challenge per person. Without this, every repeat of the
           -- password step left another usable challenge standing, so the
           -- five-guess cap was five guesses PER challenge and an attacker
           -- could hold as many as they cared to mint.
           update auth.mfa_challenges set used_at = now()
...
async function createSession(client, userId, userAgent) {
  const token = newToken();
  // Same lookup sessionFromToken uses to find a role: auth.profile_for($1)
  // lives in the user's own centre schema, and a kiosk account is the only
  // role whose session outlives a shift (see KIOSK_SESSION_TTL_DAYS above).
  const { rows: forTtl } = await client.query(`select role from auth.profile_for($1)`, [userId]);
  const ttlMs = forTtl[0]?.role === 'kiosk'
    ? KIOSK_SESSION_TTL_DAYS * 86400_000
    : SESSION_TTL_HOURS * 3600_000;
...
async function completeMfa({ challengeId, code, trustDevice, ip, userAgent } = {}) {
  ...
  return withOwner(async (client) => {
    // One statement: the increment IS the gate. This used to be a SELECT with
    // `for update of c` followed by a separate UPDATE — but withOwner() hands
    // out a bare pooled client and opens no transaction (unlike withIdentity
    // and withTransaction), so the row lock was taken and released inside that
    // one autocommit statement. Concurrent requests all read the same
    // pre-increment value and all passed a cap that is the only hard bound on
    // a six-digit secret. Incrementing in the same statement that filters on
    // `attempts < $2` cannot be raced: each caller gets its own row version.
    const { rows } = await client.query(
      `update auth.mfa_challenges c
          set attempts = c.attempts + 1
         from auth.users u
        where c.id = $1 and u.id = c.user_id
          and c.used_at is null and c.expires_at > now() and c.attempts < $2
        returning c.id, c.user_id, c.code_sha256, u.email`,
...
//
// The kiosk role (migration 051) is a second, narrower gate on top of the
// same 401 check: a shared tablet by the door, logged in once and left
// running, must never reach anything a guard/supervisor/admin session can —
// not the residents table, not the register, not settings. It may reach
// exactly its own two routes (routes/kiosk.js, mounted at /api/kiosk) and
// read or end its own session (GET/DELETE /api/session), so the tablet can
// learn its role and site name and, eventually, be signed out.
//
// req.originalUrl, not req.path: this function runs in two different mount
// contexts — the app-level `app.use('/api', requireSession)` (where req.path
// is relative to /api, e.g. '/kiosk/search') and the two routes in
// routes/session.js that call it directly (where req.path is relative to
// THAT router's own mount, e.g. '/' for GET /api/session). req.originalUrl
// is the one thing both contexts agree on: the full path from the app root,
// unrewritten by any router's mounting.
function requireSession(req, res, next) {
  if (!req.session) return res.status(401).json({ error: 'Not signed in' });
  if (req.session.role === 'kiosk') {
    const path = req.originalUrl.split('?')[0];
    const ownSession = path === '/api/session' && (req.method === 'GET' || req.method === 'DELETE');
    if (!path.startsWith('/api/kiosk/') && !ownSession) {
      return res.status(403).json({ error: 'This login can only check residents in.' });
    }
  }
  next();
}
