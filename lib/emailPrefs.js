...
//
// Two are sent today: the Sunday report and the nightly email (054). The
// nightly email reads the safeguarding_alert tick — one tick, one audience,
// per the 054 design — so stopping it clears that tick.
//
...
// `retired` marks the two no longer sent: "stop all site emails" writes
// rows only for the kinds that go out (routes/unsubscribe.js), since a row
// for an email nobody sends is a staff-card line that means nothing.
const KINDS = Object.freeze({
  ...
  safeguarding_alert: Object.freeze({ name: 'the nightly safeguarding alert (now the nightly email)',       tick: 'safeguarding_alert', retired: true }),
  house_rules:        Object.freeze({ name: 'the nightly House Rules reminder (now the nightly email)',     tick: 'safeguarding_alert', retired: true }),
});
...
// The kinds still sent — what "stop all" means.
function currentKinds() {
  return Object.keys(KINDS).filter((k) => !KINDS[k].retired);
}
...
// safeguarding_alert (054) — the nightly email and the two retired kinds —
// so the same "multiple assignments" rule applies to the ticks as to the
// kinds.
...
module.exports = { KINDS, isKind, currentKinds, kindsForTick, urlFor, headersFor, slugForSchema, slugForUser, keyFor, linkFor, optOut, optIn, optOutsFor };
