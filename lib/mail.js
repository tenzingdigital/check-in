...
/* ---------------------------------------------------------------------------
   ...
   `rows` is the plain label/value table (a count per line, no names).
   `sections` (055) is for a composer that names residents in the body: each
   `{ label, value, lines, tone }` renders as its own bordered block, the
   label and value as a heading row and `lines` — one bullet per string —
   beneath it, after `rows` and before the CTA; `tone: 'attention'` paints
   the block in the same amber as the figure, for a section the reader must
   not skim past. A section with no lines renders nothing,
   so a composer can pass all four sections every time and let empty ones
...
function layout({ siteName, heading, figure, paragraphs = [], rows = [], sections = [], cta, notes = [], unsubscribe, footer }) {
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:560px;background:#ffffff;border:1px solid ${LINE};border-radius:14px;overflow:hidden;">
    ...
    <tr><td style="padding:24px;">
      ...
      ${sections.filter((s) => s.lines && s.lines.length).map((s) => {
        ...
        const bg = t ? `background:${t.bg};` : '', edge = t ? t.border : LINE, ink = t ? t.ink : INK;
        return `
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="${bg}border:1px solid ${edge};border-radius:10px;margin:0 0 18px;">
        <tr>
        ...
        <tr><td colspan="2" style="padding:9px 16px;border-top:1px solid ${edge};font:15px/1.45 ${FONT};color:${ink};">${escapeHtml(l)}</td></tr>`).join('')}
      </table>`;
      }).join('')}
      ${href ? `
