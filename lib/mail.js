...
async function send({ to, subject, text, html }) {
  ...
  if (process.env.HUT_MAIL_SINK === '1') {
    (global.__mailSink = global.__mailSink || []).push({ to, subject, text, html });
...
function layout({ siteName, heading, figure, paragraphs = [], rows = [], cta, notes = [] }) {
...
<tr><td align="center">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:560px;background:#ffffff;border:1px solid ${LINE};border-radius:14px;overflow:hidden;">
    ...
    <tr><td style="background:#fbfcfe;border-top:1px solid ${LINE};padding:14px 24px;">
      <p style="margin:0;font:13px/1.5 ${FONT};color:${MUTED};">You are receiving this because your CheckSteady account is ticked for it. It carries counts only — the detail stays behind your login.</p>
    </td></tr>
  </table>
</td></tr>
</table>
...
module.exports = { send, resetEmail, inviteEmail, codeEmail, isConfigured, publicUrl, layout, escapeHtml };
