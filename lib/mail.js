...
async function send({ to, subject, text }) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      // back, never our request — so this cannot leak RESEND_API_KEY or any
      // other request header — but it is untrusted and unbounded, so it is
      // read defensively and capped well short of flooding the log with,
      // say, an HTML error page. Reading it can itself fail; that must not
      // stop the status code from being logged or turn into a throw.
      let detail = '';
      try {
...
module.exports = { send, resetEmail, inviteEmail, codeEmail, isConfigured, publicUrl };
