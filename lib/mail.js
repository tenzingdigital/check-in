...
async function send({ to, subject, text, html, headers }) {
  ...
  if (process.env.HUT_MAIL_SINK === '1') {
    ...
    (global.__mailSink = global.__mailSink || []).push({ to, subject, text, html, headers: sunk });
...
function layout({ siteName, heading, figure, paragraphs = [], rows = [], cta, notes = [], unsubscribe }) {
