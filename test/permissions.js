...
module.exports = [
  ...
  { area: 'Administration', name: 'Tick or untick whether a staff member receives the overnight safeguarding alert', method: 'POST', path: (fx) => `/api/staff/${fx.safeguardingStaffId}/safeguarding-alert`, body: () => ({ on: true }), expect: ADMIN, fresh: 'safeguardingStaff' },
  { area: 'Administration', name: 'Put a staff member back on the nightly House Rules reminder after they unsubscribed', method: 'POST', path: (fx) => `/api/staff/${fx.safeguardingStaffId}/house-rules`, body: () => ({ on: true }), expect: ADMIN, fresh: 'safeguardingStaff' },
