...
/* ============================================================================
   auth.js — passwords, sessions and cookies.
...
export const COOKIE_SECURE = process.env.HUT_ALLOW_INSECURE_COOKIE !== "1";
...
export const COOKIE_NAME = COOKIE_SECURE ? "__Host-hut_session" : "hut_session";
...
export function lockedOut(email, ip) {
 ...
 */
export async function signIn(email, password, { ip, userAgent } = {}) {
  ...
  return withOwner(async (client) => {
    ...
    const { rows: profile } = await client.query(
      `select full_name, role, active from public.profiles where id = $1`,
      [rows[0].id],
    );
    ...
    const { rows } = await client.query(
      `update auth.sessions s
        ...
        where s.token_sha256 = $1
          ...
          and p.id = s.user_id
          and p.active
      returning s.user_id, p.full_name, p.role`,
      [tokenHash(token)],
    );
    if (!rows[0]) return null;
    return { userId: rows[0].user_id, fullName: rows[0].full_name, role: rows[0].role };
  });
}
