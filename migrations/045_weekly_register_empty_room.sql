-- A room with every contracted bed free read exactly like one with just a
-- spare bed or two: "3 of 3 beds free." Brighton's own template treats a
-- fully empty room as its own thing worth calling out plainly — "available
-- immediately for new families" — because it can take a new arrival (or a
-- whole family) without moving anyone else, which a room with one bed free
-- usually cannot. Only the wording changes; which rooms qualify (not
-- archived, not under maintenance, at least one bed free) is unchanged.
